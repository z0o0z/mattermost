...
// Preference name for dismissing activity warning per channel
const ABAC_ACTIVITY_WARNING_PREFERENCE_PREFIX = 'abac_activity_warning_dismissed_';
...
function ChannelSettingsAccessRulesTab({
    ...
    const activityWarningDismissed = useSelector((state: GlobalState) =>
        getBool(state, Preferences.CATEGORY_ADVANCED_SETTINGS, `${ABAC_ACTIVITY_WARNING_PREFERENCE_PREFIX}${channel.id}`, false),
    );
    ...
    const [activityDelta, setActivityDelta] = useState<{
        new_messages: number;
        new_members: number;
        ...
        try {
            ...
            if (channel.id && !activityWarningDismissed && changes.toAdd.length > 0) {
                try {
                    ...
                    if (warningResult.data?.should_show_warning && warningResult.data.activity_delta) {
                        // Show activity warning modal
                        setActivityDelta(warningResult.data.activity_delta);
                ...
                // Skipping activity warning check (dismissed or no users being added)
            }
    ...
    const handleSaveChanges = useCallback(async () => {
        ...
        try {
            ...
            if (result === 'saved') {
                ...
                // Do nothing to the panel state - confirmation modal will handle next steps
            }
        } finally {
            saveInProgressRef.current = false;
        }
        ...
        if (currentUserId) {
            try {
                await actions.savePreferences(currentUserId, [{
                    ...
                    name: `${ABAC_ACTIVITY_WARNING_PREFERENCE_PREFIX}${channel.id}`,
                    value: 'true',
                }]);
            } catch (error) {
                // eslint-disable-next-line no-console
